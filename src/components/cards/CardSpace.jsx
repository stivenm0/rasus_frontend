...
function CardSpace({space, setOpenDelete, setOpenEdit, setIsEditing, setSpace }) {
  return (
    <div className="p-1 mb-2 bg-gray-700 border rounded-md">
          <div className="flex flex-wrap justify-between w-full gap-2">
            <Link to={`/perfil/${space.slug}`} className="font-bold text-gray-300">{space.name}</Link>
            <p>
        ...
        onClick={()=> {
          setOpenEdit(true)
          setIsEditing(true)
          setSpace({id: space.id , name: space.name, description: space.description})
        }} >editar</button>
